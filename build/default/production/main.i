...
void main(void) {
    ...
    char buf[16];
    int weekday, bwdcheck=0;

    while(1){
        seccount(&sec);
        mincount(&min);
        datecount(&hour, &day, &month, &year, &bwdcheck);
        weekday = weekdaycheck(day, month, year);
        dst(&hour, day, month, weekday, &bwdcheck);
        LEDarray_disp_bin(hour);
        LCD_Disp(buf,year,month,day,hour,min,sec,weekday);
